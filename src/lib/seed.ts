...
function generate(overrides: Map<string, { min: number; max: number }>, behind: Map<string, number>, debug?: string[]): WorkspaceSnapshot {
  ...
  activity.push({ id: "act_agent_1", type: "agent.action", message: "Strato updated lead time on 9 Mouser parts (approved by Maya Chen)", actorId: "u_maya", actorName: "Maya Chen", meta: { count: 9 }, createdAt: iso(1, 16, now) });
  ...
  return {
    ...
    quotes: [],
    channelTombstones: [],
    quoteTemplates: [], orderTemplates: [], customers: [], events: [], purchaseOrders: [], purchaseOrderTemplates: [],
  };
}
