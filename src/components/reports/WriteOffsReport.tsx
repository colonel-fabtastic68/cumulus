...
export function WriteOffsReport() {
  ...
  const columns = useMemo<Column<Row>[]>(
    () => [
      ...
      { key: "by", header: "By", hideBelow: "lg", render: (r) => <span className="text-text-secondary">{r.by?.name ?? (r.movement.createdBy === "agent" ? "Strato" : <Dash />)}</span>, sortValue: (r) => r.by?.name ?? "" },
    ],
    [currency],
  );
      ...
      <ReportHeader
        ...
        actions={
          <>
            ...
            <AskAgentButton prompt={agentPrompt} label="Ask Strato" disabled={all.length === 0} />
          </>
        }
      />
