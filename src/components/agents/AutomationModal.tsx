...
function AutomationForm({ onClose, automation, onSave, onDelete }: AutomationModalProps) {
  ...
  const promptError = touched && !prompt.trim() ? "Tell Strato what to do" : undefined;
  ...
  return (
    <>
      <Modal
        ...
        subtitle="A saved prompt Strato runs on a schedule or on demand."
        footer={
          <>
            {!isNew && onDelete && (
      ...
      >
        ...
        >
          ...
          <TextArea
            ...
            rows={5}
            error={promptError}
            help={promptError ? undefined : "Written as if you typed it into Strato. Be precise about scope and what to do with the result."}
          />
