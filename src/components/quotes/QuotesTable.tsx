...
export function QuotesTable({ quotes, currency, onSelect, onNew }: { quotes: Quote[]; currency: string; onSelect: (q: Quote) => void; onNew?: () => void }) {
  ...
  return (
    <Table
      ...
      emptyState={<EmptyState icon={<FileText />} title={quotes.length ? "No quotes match" : "No quotes yet"} description={quotes.length ? "Try another filter." : "Describe what a customer wants and Strato prices it from your items and labor rate, or build one line by line."} action={onNew && !quotes.length ? <Button variant="primary" onClick={onNew}>New quote</Button> : undefined} />}
    />
  );
}
