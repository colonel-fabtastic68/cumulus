...
| Strato · Chat | Full-page chat with saved conversations for the whole team, plus the docked panel (⌘J) on every page. |
| Strato · Quotes | Quotes priced from your items, labour rate and margins; Strato drafts lines from a sentence; print/PDF, mark sent/accepted/declined, raise the sales order on acceptance. |
| Strato · Projections | Stock, demand, value and cost projected from the ledger for the company, a category or a SKU; seasonality, growth and price/cost scenarios, or describe a what-if in words. |
| 24 · Cross-referencing / part interchange | Item → Cross-references: OEM, aftermarket, competitor, supplier numbers and nicknames. Search, scan and Strato all match them; superseded numbers resolve to the replacement. |
...
src/
  ...
  lib/quotes.ts         quote totals, drafting from Strato, status flow, order from quote
